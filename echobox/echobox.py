...
# version strings used by the what(1) and ident(1) commands:
SCCS_ID = "@(#) echobox - write arguments in a box v1.0.0 (May 1st, 2021) by Hubert Tournier"
RCS_ID = "$Id: echobox - write arguments in a box v1.0.0 (May 1st, 2021) by Hubert Tournier $"
...
def display_help():
    ...
    print("  -b|--basic-char    Character to use for basic style: '%s'" % parameters["Basic char"])
    print("  -d|--debug         Enable debug mode")
    print("  -f|--fill-char     Character to use to fill background: '%s'"
        % parameters["Fill char"])
    print("  -h|--help|-?       Print usage and this help message and exit")
    print("  -i|--inter-lines   Blank lines around the text: %d" % parameters["Internal lines"])
    print("  -S|--style         Style to use: %s" % parameters["Style"])
    print("  -s|--spaces        Spaces around the text: %d" % parameters["Surrounding spaces"])
    print("  -t|--trail-lines   Blank lines after the box: %d" % parameters["Trailing lines"])
...
def process_environment_variables():
    ...
    if "ECHOBOX_BASIC_CHAR" in os.environ.keys() \
    ...
    if "ECHOBOX_FILL_CHAR" in os.environ.keys() \
    ...
    if "ECHOBOX_SPACES" in os.environ.keys():
        if os.environ["ECHOBOX_SPACES"].isdigit():
            ...
            logging.error("Non numeric value in the ECHOBOX_SPACES environment variable")
    ...
    if "ECHOBOX_INTER_LINES" in os.environ.keys():
        if os.environ["ECHOBOX_INTER_LINES"].isdigit():
            ...
            logging.error("Non numeric value in the ECHOBOX_INTER_LINES environment variable")
    ...
    if "ECHOBOX_TRAIL_LINES" in os.environ.keys():
        if os.environ["ECHOBOX_TRAIL_LINES"].isdigit():
            ...
            logging.error("Non numeric value in the ECHOBOX_TRAIL_LINES environment variable")
    ...
    and os.environ["COLUMNS"].isdigit():
...
def process_command_line():
    ...
    try:
        ...
        options, remaining_arguments = getopt.getopt(
			...
			"a:b:df:hi:S:s:t:v?",
			[
				"align=", "basic-char=", "debug", "fill-char=", "help", "inter-lines=", "style=",
                "spaces=", "trail-lines=", "version"
			]
			)
    ...
    for option, argument in options:
        ...
        if option in ("-a", "--align"):
            if argument.lower() in ("left", "center", "middle", "right"):
                ...
                logging.critical("-a/--align parameter must be one of: left, middle, center, right")
                sys.exit(1)
            ...
            if argument.isdigit() and int(argument) >= 0:
                ...
                logging.critical("-i/--inter-lines parameter must be a positive integer")
                sys.exit(1)
            ...
            if argument.lower() in STYLES.keys():
                ...
                logging.critical("-S/--style parameter must be one of: %s", " ".join(STYLES.keys()))
                sys.exit(1)
            ...
            if argument.isdigit() and int(argument) >= 0:
                ...
                logging.critical("-t/--trail-lines parameter must be a positive integer")
                sys.exit(1)

        elif option in ("-v", "--version"):
            print(SCCS_ID.replace("@(#) ", ""))
            sys.exit(0)
...
logging.debug('text="%s" width=%d columns=%d indent=%d', text, len(text), columns, text_indent)
